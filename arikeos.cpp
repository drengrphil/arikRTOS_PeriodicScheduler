...
// Cooperative means a task informs processor
// when it is done executing even if its time quanta is
// not over yet.
void ArikeOS::osCurrentThreadYield(void){
    // Reset sys_tick register
    mcal::reg::reg_access<uint32_t,
				uint32_t,
				mcal::reg::systick_val,
				0x00>::reg_set();
